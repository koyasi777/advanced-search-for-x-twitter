...
// @version      6.4.9
// @description      No need to memorize search commands anymore. Adds a feature-rich floating window to X.com (Twitter) that combines an easy-to-use advanced search UI, search history, saved searches, local tweet (post) bookmarks with tags, regex-based muting, and folder-based account and list management.
// @description:ja   検索コマンドはもう覚える必要なし。誰にでも使いやすい高度な検索UI、検索履歴、検索条件の保存、ツイート（投稿）をタグで管理できるローカルお気に入り機能、正規表現対応のミュート、フォルダー分け対応のアカウント／リスト管理機能などを統合した超多機能フローティングウィンドウを X.com（Twitter）に追加します。
// @description:en   No need to memorize search commands anymore. Adds a feature-rich floating window to X.com (Twitter) that combines an easy-to-use advanced search UI, search history, saved searches, local tweet (post) bookmarks with tags, regex-based muting, and folder-based account and list management.
// @description:zh-CN 无需再死记硬背搜索命令。为 X.com（Twitter）添加一个超多功能浮动窗口，集成易用的高级搜索界面、搜索历史、已保存的搜索条件、支持为推文（帖子）添加标签的本地收藏、基于正则表达式的屏蔽，以及支持按文件夹管理的账号和列表功能。
// @description:zh-TW 無需再死記硬背搜尋指令。為 X.com（Twitter）增加一個超多功能懸浮視窗，整合易用的高級搜尋介面、搜尋紀錄、已保存的搜尋條件、可用標籤管理推文（貼文）的本地收藏、正則表示式過濾，以及支援以資料夾分類的帳號和列表管理功能。
// @description:ko   더 이상 검색 명령어를 외울 필요 없습니다. X.com(Twitter)에 누구나 쉽게 사용할 수 있는 고급 검색 UI, 검색 기록, 검색 조건 저장, 트윗(게시글)을 태그로 관리할 수 있는 로컬 즐겨찾기 기능, 정규식 음소거, 폴더 분류가 가능한 계정 및 리스트 관리 기능 등을 통합한 다기능 플로팅 창을 추가합니다.
// @description:fr   Plus besoin de mémoriser les commandes de recherche. Ajoute à X.com (Twitter) une fenêtre flottante très complète regroupant une interface de recherche avancée et facile à utiliser, l’historique, les recherches enregistrées, des favoris locaux pour les tweets (publications) avec tags, un masquage par expressions régulières (regex) et une gestion des comptes et listes avec classement par dossiers.
// @description:es   ¡Olvídate de memorizar comandos de búsqueda! Añade a X.com (Twitter) una ventana flotante multifuncional con una interfaz de búsqueda avanzada y fácil de usar, historial, búsquedas guardadas, favoritos locales de tuits (publicaciones) con etiquetas, silenciado mediante expresiones regulares (regex) y gestión de cuentas y listas con organización por carpetas.
// @description:de   Kein Auswendiglernen von Suchbefehlen mehr! Fügt X.com (Twitter) ein multifunktionales schwebendes Fenster hinzu, das eine leicht zu bedienende erweiterte Suchoberfläche, Suchverlauf, gespeicherte Suchanfragen, lokale Lesezeichen für Tweets (Posts) mit Tags, Stummschaltung per regulären Ausdrücken (Regex) und eine ordnerbasierte Konten- und Listenverwaltung vereint.
// @description:pt-BR Não precisa mais decorar comandos de busca! Adiciona ao X.com (Twitter) uma janela flutuante multifuncional com uma interface de busca avançada e fácil de usar, histórico, buscas salvas, favoritos locais de tweets (posts) com tags, silenciamento por expressões regulares (regex) e gestão de contas e listas com organização em pastas.
// @description:ru   Больше не нужно запоминать поисковые команды! Добавляет на X.com (Twitter) многофункциональное плавающее окно с удобным интерфейсом расширенного поиска, историей, сохранёнными запросами, локальными закладками твитов (постов) с тегами, фильтрацией по регулярным выражениям (regex) и управлением аккаунтами и списками с организацией по папкам.
